...
static bool StrongPassword(string password){//Cria uma Função "StrongPassword" que recebe uma String.
    ...
    //".IsMatch" é um metodo da Class "Regex" que serve pra verifica se a String informada atende ao Padrão tbm informado "Regex.IsMatch(String, Padrão)".

    if (password.Length<8){return false;}//O ".length" pega o tamanho da String e compara com 8(num min de caracteres).
    if(!Regex.IsMatch(password, "[A-Z]")){return false;}//Negando(!) o Regex.IsMatch verifica se as Letras Maiusculas de A-Z não existem na String, se não existirem Retorna "False".
    if(!Regex.IsMatch(password, "[0-9]")){return false;}//Negando(!) o Regex.IsMatch verifica se numeros não existem na String, se não existirem Retorna "False".
    if(!Regex.IsMatch(password, @"[!@#\$%\^&\*\(\),\.\?""\:\{\}\|\<\>]")){return false;}//Negando(!) o Regex.IsMatch verifica se os caracteres especiais não estão presentes na string, se não existirem Retorna "False".
